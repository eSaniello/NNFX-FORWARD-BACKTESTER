...
print("Starting Backward SHITs Forwardtest server...")
...
print("Waiting for testers to connect...")
...
print("Total of", len(signals), "testers connected...")
print()
    ...
    while True:
        try:
            ...
            if len(signals) == clients:
                print("Recieved all signals from client")
                print(signals)
                break

        except zmq.error.Again:
            time.sleep(.001)
            # kbfunc() # poll keyboard for ctrl-z to exit (or move on to testing)

print(signals)


# OK time to send message to subscriber
# sub.send_string(dsaf)

#topic = "AUDUSD"
#messagedata = "SHITBERGER"
